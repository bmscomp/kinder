...
detect-zscaler: ## Detect Zscaler proxy settings (macOS only)
	@echo "$(GREEN)Detecting Zscaler proxy configuration...$(NC)"
	@./scripts/detect-zscaler.sh
...
configure-dns: ## Configure DNS from host machine to all nodes
	...
	if [ -z "$$DNS_SERVERS" ]; then \
		echo "$(YELLOW)No DNS servers found on host$(NC)"; \
		exit 1; \
	fi; \
	echo "$(GREEN)Host DNS servers: $$DNS_SERVERS$(NC)"; \
	echo ""; \
	for NODE in $(CLUSTER_NAME)-control-plane $(CLUSTER_NAME)-worker $(CLUSTER_NAME)-worker2; do \
		echo "$(GREEN)Configuring DNS for $$NODE...$(NC)"; \
		...
		echo "$(GREEN)✓ DNS configured for $$NODE$(NC)"; \
		docker exec $$NODE cat /etc/resolv.conf | grep -E "^nameserver|^search|^options"; \
		echo ""; \
	done
	@echo "$(GREEN)DNS configuration completed for all nodes$(NC)"

check-dns: ## Check DNS configuration on all nodes
	@echo "$(GREEN)Checking DNS configuration on all nodes...$(NC)"
	@echo ""
	@for NODE in $(CLUSTER_NAME)-control-plane $(CLUSTER_NAME)-worker $(CLUSTER_NAME)-worker2; do \
		echo "$(GREEN)=== $$NODE ===$(NC)"; \
		docker exec $$NODE cat /etc/resolv.conf 2>/dev/null || echo "  No resolv.conf found"; \
		echo ""; \
		echo "$(YELLOW)DNS resolution test:$(NC)"; \
		docker exec $$NODE nslookup kubernetes.default.svc.cluster.local 2>/dev/null || echo "  DNS test failed"; \
		echo ""; \
		echo "---"; \
		echo ""; \
	done
